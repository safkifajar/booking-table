  ...
  return (
    <div className="space-y-2">
      {items.map((item, idx) => {
        ...
        const barWidth = (item.total_revenue / maxRevenue) * 100;
        return (
          <div
            key={item.menu_item_id}
            className="relative p-3 rounded-md bg-muted/20 border border-border overflow-hidden"
          >
            {/* Background bar */}
            <div
            ...
            <div className="relative flex items-center gap-3">
              <span className="w-6 text-xs font-bold text-primary/70 shrink-0">
                #{idx + 1}
              </span>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate">{item.name}</p>
                <p className="text-[10px] text-muted-foreground">
                  {item.category_name} · {item.total_qty} pcs · {item.transaction_count}{" "}
                  transaksi
                </p>
              </div>
