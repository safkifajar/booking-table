...
import { formatIDR, getActionErrorMessage, cn, initials } from "@/lib/utils";
...
export function OpenTableForm({
  ...
  function handleSlotClick(iso: string) {
    ...
    if (!selectedSlot) {
      // Belum ada pilihan → mulai (1 slot).
      setSelectedSlot(iso);
      setSelectedEnd("");
      return;
    }
    const startMs = new Date(selectedSlot).getTime();
    const endMs = selectedEnd
    ...
    // Klik setelah rentang → perpanjang: selesai = jam diklik + 1 slot.
...
function UserInvitePicker({
  ...
  /** "join" = friends (langsung gabung), "invite" = invite_only (perlu terima) */
  mode: "join" | "invite";
  ...
  function handleQueryChange(q: string) {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const rows = await searchInviteCandidates(q);
